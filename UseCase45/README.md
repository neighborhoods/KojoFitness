# Defines the fitness of Kōjō for
 ...
 # Prepare Kōjō
 docker-compose exec kojo_fitness bash -c 'cd UseCase45; composer install';
 docker-compose exec kojo_fitness bash -c 'cd UseCase45; ./vendor/bin/kojo db:setup:install $PWD/src/V1/Environment/';
 docker-compose exec kojo_fitness bash -c 'cd UseCase45; php ./bin/setup-worker.php';
 
 # Create messages for Kōjō to delete
 docker-compose exec kojo_fitness bash -c 'cd UseCase45; php ./bin/create-messages.php';
 
 # Run Kōjō to delete the messages and colorize the events
 docker-compose exec kojo_fitness bash -c 'cd UseCase45; ./vendor/bin/kojo process:pool:server:start $PWD/src/V1/Environment/' |\
 ...
 # Delete the Kōjō Tables and clear redis
     docker-compose exec kojo_fitness bash -c 'cd UseCase45; ./vendor/bin/kojo db:tear_down:uninstall $PWD/src/V1/Environment/';
